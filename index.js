...
function indekseGoreFenomen(fenomenler, indeks) {
  let kisi = (indeks + ". indekste bulunan fenomen: " + fenomenler[indeks].profile);
  return kisi;
...
function profilListesi(fenomenler) {
  let kopyfen = [...fenomenler];
  let yenidizin = [];
  for (let i=0; i<kopyfen.length; i++) {
    yenidizin.push(kopyfen[i].profile);
  }
  return yenidizin;
}
...
function fenomenSil(fenomenler, indeks) {
  let kopyasil = [...fenomenler];
  kopyasil.splice(indeks,1);
  return kopyasil;
}
...
function fenomenGonderimSayisi(fenomenler, profile){
  ...
  for(let i=0; i<fenomenler.length; i++) {
    if (fenomenler[i].profile == profile) {
      fenomengonderim = fenomenler[i].posts;
    }
  }
  return fenomengonderim;
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenler, platform) {
  ...
  for (var i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].platform.includes(platform)) {
     filtArr.push(fenomenler[i])
    }
  }
  let sonuc = filtArr.sort((a,b) => b.posts - a.posts);
  return sonuc[0].profile;
}
